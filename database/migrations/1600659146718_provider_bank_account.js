'use strict';

/** @type {import('@adonisjs/lucid/src/Schema')} */
const Schema = use('Schema');
const Database = use('Database');

class ProviderBankAccountSchema extends Schema {

  async up () {
  Database.raw("SET FOREIGN_KEY_CHECKS=0").then( () => {
    this.create('provider_bank_account', (table) => {
    			table.increments('id').unsigned();
					table.integer('provider_id').references('id').inTable('provider').unsigned();
					table.string('provider_name', 100);
					table.string('account_number', 100);
					table.string('recipient_code', 100).nullable();
					table.integer('bank_id').references('id').inTable('bank').unsigned();
					table.integer('bank_type_id').references('id').inTable('bank_type').unsigned();
					table.integer('created_by').references('id').inTable('user').unsigned().nullable();
					table.integer('updated_by').unsigned().nullable();
					table.timestamp('created_at').nullable();
					table.timestamp('updated_at').nullable();
					table.integer('active_status', 3).references('id').inTable('active_status').unsigned().defaultTo('1');
					
    });
    }).finally(function () {
      return Database.raw('SET FOREIGN_KEY_CHECKS=1;');
    });
  }

  async down () {
  Database.raw("SET FOREIGN_KEY_CHECKS=0").then( () => {
    this.dropIfExists('provider_bank_account');
    }).finally(function () {
      return Database.raw('SET FOREIGN_KEY_CHECKS=1;');
    });
  }
}

module.exports = ProviderBankAccountSchema
